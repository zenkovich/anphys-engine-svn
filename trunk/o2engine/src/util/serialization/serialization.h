...
{
	virtual bool serialize(pugi::xml_node& xmlNode, cSerializeType::v type, sharedPtr(cLogStream) log = NULL) = 0;
};
...
struct cSerialization
{
	...
	/** Loading xml data to @doc from file. Puts errors in @log, if @log is null - using global gLog. */
	static bool loadData(pugi::xml_document& doc, cInFile& file, sharedPtr(cLogStream) log = NULL);
...
//NOTE: Do not use that functions !
	...
	template<typename T>
	static bool serializeArrIn(pugi::xml_node& xmlNode, const string& id, T* arr, int count, sharedPtr(cLogStream) log = NULL)
	{
		bool res = true;
	...
	template<typename T>
	static bool serializeArrOut(pugi::xml_node& xmlNode, const string& id, T* arr, int count, sharedPtr(cLogStream) log = NULL)
	{
		bool res = true;
...
#define SERIALIZE_INHERITED_METHOD_IMPL(CLASS) \
	bool CLASS::serializeInh(pugi::xml_node& xmlNode, cSerializeType::v type, sharedPtr(cLogStream) log)       
