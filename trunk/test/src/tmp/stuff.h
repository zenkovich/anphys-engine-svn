...
struct vec3
{
	...
	inline float len() const { return sqrtf((*this)*(*this)); }
	inline vec3 normalize() const { float ln = len(); if (ln > 0.00001f) return *this/ln; else return vec3(0); }
...
{
	return mat3x3( matrix[0], matrix[2], matrix[1],
...
{
	out[0] = in.m[0][0]; out[1] = in.m[0][2]; out[2] = in.m[0][1];
	out[3] = in.m[2][0]; out[4] = in.m[2][2]; out[5] = in.m[2][1];
	out[6] = in.m[1][0]; out[7] = in.m[1][2]; out[8] = in.m[1][1];
}
...
{
	vec3          mPosition;
	unsigned char mFrictionTableId;

	lVertex():mFrictionTableId(1) {}
	lVertex(const vec3& pos, unsigned char frictionCoef = 1, float bounceCoef = 1.0f):
...
struct lPolygon
{
	...
	{		
		vec3 pa = vmask((float*)&verticies[c]->mPosition);
		vec3 pb = vmask((float*)&verticies[b]->mPosition);
		vec3 pc = vmask((float*)&verticies[a]->mPosition);
		vec3 n = vmask((float*)&norm);
		n *= -1.0f;

		if ((pa - pb)*(pa - pb) < 0.001f ||
			(pb - pc)*(pb - pc) < 0.001f ||
			(pa - pc)*(pa - pc) < 0.001f)
			return false;

		if ((bottom - pa)*(n^(pb - pa)) > 0 ||
			(bottom - pb)*(n^(pc - pb)) > 0 ||
			(bottom - pc)*(n^(pa - pc)) > 0)
		{
			return false;
		...
		float ndepth = (pa - bottom)*n;

		*pnorm = n;
		*depth = ndepth;
		*point = bottom + n*ndepth;
...
inline void matrix2quat(float* mt, float* ret)
{
	float m[3][3];
	m[0][0] = mt[0]; m[0][1] = mt[1]; m[0][2] = mt[2];
	m[1][0] = mt[3]; m[1][1] = mt[4]; m[1][2] = mt[5];
	m[2][0] = mt[6]; m[2][1] = mt[7]; m[2][2] = mt[8];
	...
	if (trace > 0)
	{
		...
		ret[3] = 0.5f*troot;
		troot = 0.5f/troot; 
		ret[0] = (m[2][1] - m[1][2])*troot;
		ret[1] = (m[0][2] - m[2][0])*troot;
		ret[2] = (m[1][0] - m[0][1])*troot;
	}
	else
	{
		...
		if (i == 0) ret[0] = 0.5f*troot;
		if (i == 1) ret[1] = 0.5f*troot;
		if (i == 2) ret[2] = 0.5f*troot;

		troot = 0.5f/troot;

		ret[3] = (m[k][j] - m[j][k])*troot;

		if (j == 0) ret[0] = (m[i][j] + m[j][i])*troot;
		if (j == 1) ret[1] = (m[i][j] + m[j][i])*troot;
		if (j == 2) ret[2] = (m[i][j] + m[j][i])*troot;
		if (k == 0) ret[0] = (m[i][k] + m[k][i])*troot;
		if (k == 1) ret[1] = (m[i][k] + m[k][i])*troot;
		if (k == 2) ret[2] = (m[i][k] + m[k][i])*troot;
	}
}
